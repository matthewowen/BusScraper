...
class transportInfo(object):
	...
	def get_content(URL):
		...
		http = httplgit@github.com:matthewowen/BusScraper.gitib2.Http()
		resp, content = http.request(URL, "GET")
		soup = BeautifulSoup(content)

		return soup

	def stop(stop_id, self):

		"""
		returns all the buses for a given stop as a list of dictionaries
		"""

		URL = "http://%s.acislive.com/pip/stop.asp?naptan=%s&textonly=1" % (self.subdomain, stop_id)
	...
	def postcode(postcode, self):

		"""
		returns all the stops near a given postcode as a list of dictionaries
		"""

		URL = "http://%s.acislive.com/web/postcode.asp?postcode=%s&textonly=1" % (self.subdomain, postcode)
		soup = get_content(URL)
	...
	def service(operator, service, destination, self, systemid):
		"""
		returns all the stops on a particular service as a list of dictionaries
		"""

		URL = "http://%s.acislive.com/web/public_service_stops.asp?service=%s&operatorid=%s&systemid=%s&goingto=%s&showall=1&textonly=1" % (self.subdomain, service, operator, systemid, destination)
		soup = get_content(URL)
